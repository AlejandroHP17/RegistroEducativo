...
import com.mx.liftechnology.core.network.apiCall.evaluation.RequestRegisterJobStudent
...
class RegisterAssignmentUseCase (
    ...
    suspend operator fun invoke(nameJob: String, typeJob: Int, date: String, studentListUI:  List<RequestStudentJobs>): ResultModel<List<ResponseStudentJobs?>?, String> {
        ...
        return runCatching { registerAssignmentRepository.executeRegisterAssignment(request) }.fold(
            onSuccess = { result ->
                when (result){
     ...
     */
    private fun handleResponse(error: NetworkError): ResultModel<List<ResponseStudentJobs?>?, String> {
        return when(error) {
            NetworkError.BAD_REQUEST -> ErrorUserResult(ModelCodeError.ERROR_VALIDATION_LOGIN)
            NetworkError.UNAUTHORIZED -> ErrorResult(ModelCodeError.ERROR_UNAUTHORIZED)
            NetworkError.NOT_FOUND -> ErrorUserResult(ModelCodeError.ERROR_VALIDATION_LOGIN)
