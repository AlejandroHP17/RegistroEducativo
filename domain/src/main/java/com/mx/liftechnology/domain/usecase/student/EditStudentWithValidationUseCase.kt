 ...
 * @property editStudentUseCase El caso de uso para ejecutar la edición de estudiante.
 *
 * @author Pelkidev
 * @version 1.0.0
 */
class EditStudentWithValidationUseCase(
    private val validateFieldsUseCase: ValidateFieldsStudentUseCase,
    private val editStudentUseCase: EditStudentUseCase
) {
    /**
     * Valida los campos del formulario de edición de estudiante y, si son válidos, ejecuta la edición.
     *
     ...
     * - El resultado de la operación de edición (solo si todas las validaciones pasaron)
    ...
    suspend operator fun invoke(
        ...
        phoneNumber: String?,
        studentId: Int?
    ): ModelValidationResult<ModelStudentData?> {
        ...
        val operationResult = editStudentUseCase.invoke(
            name = name ?: "",
            lastName = lastName ?: "",
