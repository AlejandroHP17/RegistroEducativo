...
class GetControlRegisterUseCaseTest {
    ...
    private lateinit var useCase: GetControlRegisterUseCase

    @Before
    fun setup() {
        localRepository = mockk()
        useCase = GetControlRegisterUseCase(localRepository)
    }

    @Test
    fun `cuando lista esta vacia retorna ErrorResult EMPTY`() = runTest {
        coEvery { localRepository.getControlRegister() } returns SuccessResult(emptyList())
    ...
    fun `cuando lista tiene elementos retorna SuccessResult`() = runTest {
        ...
        coEvery { localRepository.getControlRegister() } returns SuccessResult(list)
    ...
    fun `cuando repositorio retorna ErrorResult se mapea a CATCH`() = runTest {
        coEvery { localRepository.getControlRegister() } returns ErrorResult(mockk())
