...
class GetListSubjectUseCaseTest {
    ...
    fun `invoke con respuesta exitosa devuelve SuccessState`() = runBlocking {
        // Preparamos el mock
        val mockSubjects = listOf(ResponseGetListSubject(1, "Matemáticas"))
        coEvery { getListSubjectRepository.executeGetListSubject(any()) } returns ResultSuccess(mockSubjects)
    ...
    fun `invoke con lista vacia devuelve ErrorUserState`() = runBlocking {
        // Preparamos el mock
        coEvery { getListSubjectRepository.executeGetListSubject(any()) } returns ResultSuccess(emptyList())
    ...
    fun `invoke con error del repositorio devuelve ErrorUserState`() = runBlocking {
        // Preparamos el mock
        coEvery { getListSubjectRepository.executeGetListSubject(any()) } returns ResultError(FailureService.BadRequest)
